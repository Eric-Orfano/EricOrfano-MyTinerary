  ...
  return (
    <>
      <main className="grow flex flex-col gap-5 justify-center items-center bg-gray-500 mt-20">
        <div className="w-full flex justify-center flex-wrap gap-5 mb-12">
          ...
          <search className="w-full flex justify-center">
            <input className="w-5/12 sm:w-80 border border-solid border-black shadow-xl" type="text" name="Name_city" onInput={handleInput} ref={inputSerch} />
          </search>
          {filters.length > 0 &&
            filters.map((city) => <CitiesCard key={city.id} cities={city} />)}
        </div>
      </main>
    </>
  );
