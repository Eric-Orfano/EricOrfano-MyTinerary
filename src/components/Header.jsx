      ...
      <>
       <header className="flex flex-row flex-wrap justify-between px-3 items-center h-20 fixed w-full top-0 z-10 bg-gray-900">
          ...
          <nav className="flex gap-5 text-white">
            {links.map((link) => (<Link  key={link.title} className={`p-2 mx-1 rounded-xl transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 hover:bg-gray-400 duration-150 ${link.active ? " bg-gray-700 bg-opacity-80" : ""}`} to={link.path}>{link.title}</Link>))}
